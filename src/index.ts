...
export class AtherSocketClient extends EventEmitter implements IAtherSocketClient {
    ...
    constructor(options: AtherOptions = {
        ...
        this.ather1 = createAther1(this.httpRequest, this.websocket);
    }

    connect() {
        return new Promise<websocket.WebSocket>((resolve, reject) => {
...
function waitForId(
    ...
    function callbackBadEdit(error: { status: "error", data: string } ) {
        callbackBad(error)
    }
